...
closeButton.addEventListener('click', function() {
    ...
    closeTarget.classList.add('closeAnime');
    contact.classList.add('Apper'); 
    let First = document.getElementById('FI1');  
    setTimeout(function() {
...
function SmothMove(event,firstId,nextRow,nextclass){
    ...
    if (input.value.length === 1) {
        if (event.key === 'Backspace'){
            ...
            if (nex !== null){
                input.disabled = true;
                nex.disabled = false;
                currentDisabledInput = nex;
            }
            addLetter(event);
        }
    } else if (input.value.length === 0) {
        let pre = input.previousElementSibling;
        if (event.key === 'Backspace') {
            if (pre !== null){
                input.disabled = true;
                pre.disabled = false;
                currentDisabledInput = pre;
            }
            removeLetter(event);
    ...
    if (event.key === 'Enter') {
        if (next !== null){
            ShowError('uncompleted word',firstId);
        } else {
            Check(firstId,nextRow,nextclass);
        }
    }
}
function Letter(e) {
...
function ShowError(ErrorText,firstElement){
    const ErrorMassege = document.getElementById('Massege');
    const Error = document.getElementById('ErrorMasseges');
    ErrorMassege.textContent = ErrorText;
    ErrorMassege.style.display = 'flex';
    Error.classList.add('showEr');
    setTimeout(function(){
        Error.classList.remove('showEr');
    }, 3000);
    let child = document.getElementById(firstElement);
    for (let i = 0; i < 5; i++) {
...
function Check(firstId,nextRow,nextclass){
    let cur = document.getElementById(firstId);
    let count = 0;
...
function Time(cor,cla,i,color,value){
    setTimeout(function() {
        cor.classList.add(cla);
        cor.style.backgroundColor = color;
        changeButtonColor(value,color);
    },i*1000); 
}

function changeButtonColor(value, color) {
  ...
  function ToWord(firstInput){ //tested
